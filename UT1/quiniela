...
<form method="post">
    ...
    <table border="1" style="width:900px; border-collapse: collapse; text-align:center;">
        ...
        if(isset($_POST['enviar'])) {
            echo "<tr><th>Partidos</th><th colspan='8'>Elecciones</th></tr>";

            $error = false; // Variable para rastrear errores en la selección.
            ...
            for($j = 0; $j < 14; $j++) {
                ...
                for($i = 0; $i < 8; $i++) {
                    ...
                    // Si no se ha seleccionado ninguna opción, establece $error en true.
                    if (empty($eleccion)) {
                        $error = true;
                    }
                }
